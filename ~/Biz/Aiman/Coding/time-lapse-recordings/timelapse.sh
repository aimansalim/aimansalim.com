...
OUTPUT_DIR="$HOME/Biz/Aiman/Coding/time-lapse-recordings/screenshots_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$OUTPUT_DIR"
...
create_video() {
  ...
  # Add timestamp overlay to images
  ...
  VIDEO_FILE="$HOME/Biz/Aiman/Coding/time-lapse-recordings/timelapse_$(date +%Y%m%d_%H%M%S).mp4"
